...
/**
 * RECONEXIÓN AUTOMÁTICA Y WATCHDOG
 */
client.on('disconnected', async (reason) => {
    console.log('⚠️ Cliente de WhatsApp DESCONECTADO:', reason);
    console.log('🔄 Saliendo del proceso para que PM2 reinicie limpiamente...');
    try {
...
// Watchdog interno: Verifica cada 5 min si el cliente está colgado
setInterval(async () => {
    try {
        if (!client || !client.info) return;
        const state = await client.getState();
        if (state !== 'CONNECTED') {
            console.log(`📡 Watchdog: Estado anormal detected (${state}). Reiniciando...`);
            process.exit(1);
        }
    } catch (e) {
        console.log('📡 Watchdog: Error verificando estado. Posible bloqueo de Puppeteer. Reiniciando...');
        process.exit(1);
    }
}, 300000);
client.on('message', async (msg) => {
    try {
        ...
        // 3. Ignorar mensajes enviados por el propio bot
